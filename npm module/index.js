...
// gets all matches that are that are not yet played
async function upcomingMatches(teamID) {
  var cheerio = require('cheerio'),
      ...
      var team = await csgo99damage.getShorthand(teamID);
          ...
          request({
              ...
              if (!err) {
                  ...
                  const result = $(".league_table_matches tr").map((i, element) => ({
                      ...
                      link: $(element).find('td:nth-of-type(2)').children().attr('href'),
                      TimeTo: $(element).find('td:nth-of-type(2)').children().attr('href'),
                  })).get()
...
// gets all matches that were initially scheduled
async function allThisSeasonMatches(teamID) {
  var cheerio = require('cheerio'),
      request = require('request');
...
// gets a list of all oppenents from the league page
// excludes the give team
async function getOpponentsCurrentSeason(teamid) {
    var cheerio = require('cheerio'),
        request = require('request');
        ...
        var team = await csgo99damage.getLongName(teamid);
...
// gets a list of all gamedays this season
//  - Team1 & Team2
//  - Score
//  - played
//  - time to
async function getGameDays(teamID)
{
    var cheerio = require('cheerio'),
        request = require('request');
            ...
            request({
                ...
                if (!err) {
                    ...
                    for(var item in result) {
                      if (item && result[item].hasOwnProperty('Team1') && result[item].hasOwnProperty('Team1')  && result[item].hasOwnProperty('score') ) {
                        if (result[item]['Team1'] == team || result[item]['Team2'] == team) {
...
var csgo99damage = {
    ...
    allThisSeasonMatches:allThisSeasonMatches,
    getOpponentsCurrentSeason:getOpponentsCurrentSeason,
